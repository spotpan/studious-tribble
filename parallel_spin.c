...
/*Yes I see a change in the timing. The statistics shows that
the timing of mutexlock is more stable than the timing of spinlock.
But sometimes the time of spinlock is less than the time of 
mutexlock.The difference between a spinlock and a mutexlock is 
that the former is busy-waiting type while the latter is 
sleep-waiting type. As a result, the spinlock is more efficient 
than mutexlock theoratically*/
